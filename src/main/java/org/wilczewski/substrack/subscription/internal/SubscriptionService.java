...
class SubscriptionService implements SubscriptionFacade {
    ...
    public UUID createSubscription(CreateSubscriptionCommand createSubscriptionCommand) {
        ...
        if (createSubscriptionCommand.endDate() != null
                && createSubscriptionCommand.startDate().isAfter(createSubscriptionCommand.endDate())) {
            throw new BadRequestException("Start date cannot be after end date");
        }
        if (!userFacade.userContainsEmail(createSubscriptionCommand.userId(), createSubscriptionCommand.emailId())) {
            throw new BadRequestException("User does not contain email");
        }
    ...
    public void updateSubscription(UpdateSubscriptionCommand updateSubscriptionCommand) {
        Subscription subscription = validateSubscriptionOwnership(
                updateSubscriptionCommand.userId(), updateSubscriptionCommand.id());
        if (updateSubscriptionCommand.endDate() != null
                && updateSubscriptionCommand.startDate().isAfter(updateSubscriptionCommand.endDate())) {
            throw new BadRequestException("Start date cannot be after end date");
        }
    ...
    public void deleteSubscription(DeleteSubscriptionCommand deleteSubscriptionCommand) {
        validateSubscriptionOwnership(
                deleteSubscriptionCommand.userId(), deleteSubscriptionCommand.subscriptionId());
    ...
    public SubscriptionResponse getSubscriptionById(GetUserSubscriptionQuery getUserSubscriptionQuery) {
        Subscription subscription = validateSubscriptionOwnership(
                getUserSubscriptionQuery.userId(), getUserSubscriptionQuery.subscriptionId());
